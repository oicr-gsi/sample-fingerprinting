...
my $USAGE="jaccard_coef.matrix.pl --list=[req] --studyname=[req] --existing_matrix=[optional] --vcf-compare=[] --datadir=[optional]";
...
my $result = GetOptions ('list=s'            => \$list, # list with filenames
                         ...
                         'datadir=s'         => \$datadir, # directory with data
...
my @old_matrices = grep{/\S+/} split(",",$oldmatrix);
...
foreach my $old (@old_matrices) {
 if (-e $old) {
  ...
  my @tempids = grep {/\S+/}  split("\t",$idstring);
  ...
  while (<OLD>) {
   ...
   $snps{$files{$temp[0]}} = $sindex ? $temp[$sindex] : &calculate_snps($temp[0]);
  }
  close OLD;
 }
}
...
foreach my $id(keys %ids) {
 ...
 foreach my $s(keys %ids) {
  ...
  my $file1 = join("/",($datadir,$ids{$id}));
  my $file2 = join("/",($datadir,$ids{$s}));
...
sub calculate_snps {
 ...
 my $result = `zcat $datadir/$file | grep -v ^# | wc -l`;
 chomp($result);
 return $result;
}
